...
class LoadGenerator:
  ...
  def __init__(self,
    ...
    if (self.num_tasks_per_thread != sys.maxsize) & (self.num_tasks !=
                                                     sys.maxsize):
      raise ValueError(
          "num_tasks_per_thread and num_tasks both can't be passed.")
  ...
  def _process_task(task, assigned_process_id, num_threads,
    ...
    logging.debug(
        'Threads started for process number: {0}'.format(assigned_process_id))
    for thread in threads:
      thread.join()
    logging.debug(
        'Threads completed for process number: {0}'.format(assigned_process_id))
  ...
  def _compute_default_post_test_metrics(self, observations):
    ...
    # cpu stats
    cpu_usage_pts = [
        np.mean(all_cpus_per) for all_cpus_per in observations['cpu_usage_pts']
    ]
  ...
  def generate_load(self, task):
    ...
    for process in processes:
      process.start()
    logging.debug('{0} number of processes started for task {1}'.format(
    ...
    while self._should_continue_test(time_pts[-1], time_pts[0],
      ...
      if log_loading & (curr_loading_idx < len(loading_checkpoints)) and (
        ...
        logging.info('Load test completed {0}% for task: {1}'.format(
            loading_percentages[curr_loading_idx] * 100, task.TASK_NAME))
        curr_loading_idx = curr_loading_idx + 1

    logging.info('Load test completed 100% for task: {0}'.format(
        task.TASK_NAME))

    for process in processes:
      process.terminate()
    logging.debug('{0} number of processes terminated for task {1}'.format(
        len(processes), task.TASK_NAME))
    ...
    return {
        ...
        'process_pids':
            process_pids,
        'tasks_results':
            LoadGenerator._convert_multiprocessing_queue_to_list(
                tasks_results_queue),
        'pre_tasks_results':
            LoadGenerator._convert_multiprocessing_queue_to_list(
